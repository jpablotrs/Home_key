...
public class PropiedadDAO implements IPropiedadDAO {
    ...
    public int modificarPropiedad(Propiedad propiedad) throws SQLException {
        String actualizacionSQL = "UPDATE propiedad SET direccion = ?, ciudad = ?, estado = ?, codigoPostal = ?, numHabitaciones = ?, numBanos = ?, numPisos = ?, cocina = ?, metrosCuadrados = ?, numPersonas = ?, alquiler = ?, compra = ?, electricidad = ?, amueblado = ?, foto = ?, Cliente_idCliente = ?, claveCatastral = ? WHERE idPropiedad = ?";
